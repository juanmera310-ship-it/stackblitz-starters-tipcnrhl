...
// Damos 3.5 segundos por cada fruta que necesitas romper. Es un ritmo rápido.
const SECONDS_PER_FRUIT = 3.5;

let fruitsHitInThisLevel = 0;
let timeLeft = 0;
let currentTarget = 10; // Meta inicial
...
// --- NUEVO: CALCULAR META SEGÚN EL NIVEL ---
// Nivel 1 = 10, Nivel 2 = 20, Nivel 3 = 30...
function getTarget(lvl) {
  return lvl * 10;
}

// --- Dificultad Dinámica (Velocidad de frutas) ---
function getDifficulty(lvl) {
  return {
    // Hacemos que la velocidad suba un poco más suave para que sea jugable con tantas frutas
    minSpeed: 3 + lvl * 0.2,
    maxSpeed: 5 + lvl * 0.4,
    // Aparición rápida para que tengas suficientes frutas para romper
    spawnRate: Math.min(0.03 + lvl * 0.004, 0.25),
    rotSpeed: 0.05 + lvl * 0.008,
  };
}

// --- Función para INICIAR / REINICIAR un Nivel ---
function startLevel(lvl) {
  level = lvl;
  fruitsHitInThisLevel = 0;

  // 1. Calcular nueva meta
  currentTarget = getTarget(level);

  // 2. Calcular tiempo basado en la meta (Ej: 10 frutas * 3.5s = 35 segundos)
  timeLeft = Math.ceil(currentTarget * SECONDS_PER_FRUIT);

  // 3. Limpiar pantalla
  bullets = [];
  fruits = [];
  particles = []; // Opcional: limpiar partículas o dejarlas por estética
...
function restartGame() {
  ...
  startLevel(1); // Volver al nivel 1
...
window.addEventListener('mousemove', (e) => {
  if (!isPaused && !isGameOver) touchX = e.clientX;
});
...
canvas.addEventListener('mousedown', (e) => {
  if (!isPaused && !isGameOver) shoot();
});
canvas.addEventListener('touchstart', (e) => {
  if (!isPaused && !isGameOver && e.target === canvas) shoot();
});
...
// --- Bucle Principal ---
let lastTime = 0;
// Inicializar el primer nivel al cargar
startLevel(1);
    ...
    bullets.forEach((bullet, bIndex) => {
      ...
      if (dist < fruit.size / 2.2 + bullet.size / 3) {
        ...
        if (fruitsHitInThisLevel >= currentTarget) {
          if (level < MAX_LEVEL) {
            startLevel(level + 1); // Subir de nivel y resetear tiempo/meta
          } else {
            levelOverlay.innerText = '¡JAM MASTER SUPREMO!';
            levelOverlay.style.display = 'block';
            isGameOver = true; // Fin del juego (Victoria)
          }
        }
